  ...
  public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo>GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo  where workVerift='审核通过' order by uploadTime desc limit 8";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo  where workVerift='审核通过'and recommend='是' order by recommendTime desc limit 6";
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by uploadTime desc)as num,workunfo.*from workinfo where activityId in @activityIds)";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName, activityId,recommend,recommendTime)"+
                    "value(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activityId,@recommend,@recommendTime)";
                sql += "SELECT @@IDENTITY";
